...
class FoodRecommender:
    ...
    def calculate_macros(self, calories: int, goal: str) -> Dict[str, float]:
        ...
        if goal not in self.MACRO_RATIOS:
            raise ValueError(f"Invalid goal: {goal}. Must be one of {list(self.MACRO_RATIOS.keys())}")
    ...
    def _filter_by_restrictions(self, foods: List[Dict], restrictions: List[str]) -> List[Dict]:
        """
        ...
        for food in foods:
            ...
            for restriction in restrictions:
                if restriction.startswith("no_"):
                    ...
                    if allergen == "nuts":
                        if "tree nuts" in food['allergens'] or "peanuts" in food['allergens']:
                            allergen_match = True
                            break
                    else:
    ...
    def _calculate_portion_calories(self, food: Dict, portion_g: float) -> Dict[str, float]:
    ...
    def build_meal(self, meal_calories: int, meal_protein_g: float, meal_carbs_g: float, 
                   meal_fat_g: float, restrictions: List[str]) -> Dict:
        """
        ...
        if protein_foods:
            ...
            portion_g = min(max((protein_needed / best_protein['protein_g']) * 100, self.MIN_PORTION), self.MAX_PORTION)
        ...
        grain_foods = self._filter_by_restrictions(
            self.foods_by_category.get('grain', []) + self.foods_by_category.get('carb', []), 
            restrictions
        )
        
        if grain_foods:
            # Prefer quinoa, oats, brown rice (complex carbs)
            complex_carbs = ['quinoa', 'oats', 'brown_rice', 'whole_wheat_pasta', 'sweet_potato']
            grain_foods.sort(key=lambda x: (x['id'] in complex_carbs, x['fiber_g']), reverse=True)
            best_grain = grain_foods[0]
            
            # Calculate portion for remaining carbs
            carbs_needed = max(meal_carbs_g - total_nutrition['carbs_g'], 0)
            portion_g = min(max((carbs_needed / best_grain['carbs_g']) * 100, self.MIN_PORTION), self.MAX_PORTION)
        ...
        if vegetable_foods:
            ...
            vegetable_foods.sort(key=lambda x: (x['id'] in veg_priority, x['fiber_g']), reverse=True)
        ...
        if total_nutrition['fat_g'] < meal_fat_g * 0.7:
            ...
            if fat_foods:
                ...
                portion_g = min(max((fat_needed / best_fat['fat_g']) * 100, 5), 30)  # Small portions for fats
    ...
    def _validate_safety(self, meal_plan: Dict, restrictions: List[str]) -> Tuple[bool, List[str]]:
        """
        ...
        for meal_name, meal_data in meal_plan.items():
            if meal_name in ['breakfast', 'lunch', 'dinner']:  # Not snack
                if meal_data['total_protein_g'] < self.MIN_PROTEIN_PER_MEAL:
                    errors.append(f"{meal_name} has insufficient protein: {meal_data['total_protein_g']}g < {self.MIN_PROTEIN_PER_MEAL}g")
        
        # Check portion sizes
        for meal_name, meal_data in meal_plan.items():
            for food in meal_data['foods']:
                if food['portion_g'] < self.MIN_PORTION or food['portion_g'] > self.MAX_PORTION:
                    if food['portion_g'] > 50:  # Ignore tiny portions like oil
                        errors.append(f"Unsafe portion in {meal_name}: {food['name']} = {food['portion_g']}g")
    ...
    def generate_plan(self, model2_output: Dict) -> Dict:
        ...
        try:
            ...
            daily_summary = {
                ...
                "protein_g": sum(meal['total_protein_g'] for meal in meal_plan.values()),
    ...
    def _format_chatbot_message(self, meal_plan: Dict, daily_summary: Dict, 
                                goal: str, target_calories: int) -> str:
        ...
        for meal_name in ['breakfast', 'lunch', 'dinner', 'snack']:
            ...
            meal_descriptions.append(f"**{meal_name.title()}**: {meal['suggested_sentence']}")
